      ...
      <div style="font-family:'DM Sans',Helvetica,Arial,sans-serif;max-width:500px;margin:0 auto;background:#00154F;color:#F5F0E8;padding:36px 32px;border-radius:6px">
        ...
        <div style="background:rgba(62,207,142,.08);border-left:3px solid #3ecf8e;padding:14px 18px;border-radius:3px;margin-bottom:20px">
          <p style="margin:0;font-size:13px;line-height:2">
            ⚡ <strong>Early drop access</strong> — 24 hours before public<br>
            🏆 <strong>Priority bidding</strong> on every auction<br>
            🛒 <strong>Members-only shop items</strong><br>
            📧 <strong>Direct alerts</strong> on every new drop
          </p>
        </div>
...
const handler = async (req, res) => {
  ...
  let event;
  try {
    // Read raw body from the request stream before any parsing occurs.
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      // Only process subscription checkouts that were paid
      ...
      // Plan was stored at checkout time — no race condition possible
      ...
      // Check if this is a brand-new subscriber before overwriting the record
      ...
      // Only increment the total count for new subscribers — not re-subscriptions
      if (!wasAlreadyActive) {
        await redis('INCR', 'subscribers:total');
      }
    ...
    if (event.type === 'invoice.payment_failed') {
      console.warn('Payment failed for Stripe customer:', event.data.object.customer);
    }
